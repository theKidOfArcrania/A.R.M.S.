...
public class PlayerInfoTest extends Application {
	...
	private void init(Stage primaryStage) {
		...
		Rectangle shieldBar = Tools.createRoundedRectangle((168) * shieldScale.getValue(), 20, 5, 5, 86, 117.5, sRatio, wRatio, hRatio, Color.WHITE, null);
		...
		Rectangle energyBar = Tools.createRoundedRectangle((168) * energyScale.getValue(), 20, 5, 5, 86, 85, sRatio, wRatio, hRatio, Color.WHITE, null);
		energyBar.widthProperty()
				.bind(energyScale.multiply(168 * wRatio));
		energyBar.setFill(energyGrad);

		Rectangle panel = Tools.createRoundedRectangle(190, 25, 5, 5, 80, 50, sRatio, wRatio, hRatio, Color.DARKGRAY.darker()
				.darker(), largeShade);

		root.getChildren()
				.addAll(name, weapons, energy, shieldEmpty, shieldBar, energyEmpty, energyBar, weaponDisplay, energyDisplay, shieldDisplay, shield, profileView,
						screw1, screw2, screw3, screw4, panel, playerRank, playerGames, opener);
		...
		scene.getStylesheets()
				.add("/armsgame/ui/test/border.css");
